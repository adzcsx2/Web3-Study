    ...
    CustomModifiers
{
     ...
     * @param poolId 池子ID
     */
    function reactivatePool(uint256 poolId) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (poolId >= poolCounter) revert PoolNotExists(poolId);
     ...
     * @param count 获取数量
     */
    function getPoolsBatch(uint256 startIndex, uint256 count) external view returns (
