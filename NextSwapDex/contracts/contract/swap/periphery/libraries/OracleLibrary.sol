...
library OracleLibrary {
    /// @notice Calculates time-weighted means of tick and liquidity for a given Nextswap V3 pool
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        ) = INextswapV3Pool(pool).observe(secondsAgos);
    ...
    /// @param pool Address of Nextswap V3 pool that we want to observe
    ...
    function getOldestObservationSecondsAgo(
        ...
        ) = INextswapV3Pool(pool).slot0();
        require(observationCardinality > 0, "NI");

        (uint32 observationTimestamp, , , bool initialized) = INextswapV3Pool(
            pool
        ...
        if (!initialized) {
            (observationTimestamp, , , ) = INextswapV3Pool(pool).observations(
                0
            );
        }
    ...
    /// @param pool Address of Nextswap V3 pool
    ...
    function getBlockStartingTickAndLiquidity(
        ...
        ) = INextswapV3Pool(pool).observations(observationIndex);
        if (observationTimestamp != uint32(block.timestamp)) {
            return (tick, INextswapV3Pool(pool).liquidity());
        }
        ...
        ) = INextswapV3Pool(pool).observations(prevIndex);
