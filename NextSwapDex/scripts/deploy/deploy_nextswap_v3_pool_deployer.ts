...
async function main() {
  ...
  console.log("\n📝 说明: NextswapV3PoolDeployer 的 deploy 方法是 internal 的");
  console.log("   只能在 NextswapV3Factory 合约中通过 createPool 调用");
  ...
  if (!fs.existsSync(deploymentPath)) {
    console.error("❌ 未找到部署记录，请先部署 NextswapV3Factory");
    return;
  }

  const deployment = JSON.parse(fs.readFileSync(deploymentPath, "utf-8"));
  const factoryAddress = deployment.contracts?.NextswapV3Factory?.proxyAddress;

  if (!factoryAddress) {
    console.error("❌ 未找到 NextswapV3Factory 地址");
    return;
  }

  console.log("✅ 找到已部署的 NextswapV3Factory:", factoryAddress);
  ...
  // 这会内部调用 NextswapV3PoolDeployer.deploy 方法
  const fee = 3000; // 0.3% 手续费
  console.log(`\n🔨 通过 Factory 创建池（手续费: ${fee / 10000}%）...`);
  console.log("   这将触发 NextswapV3PoolDeployer.deploy() 的调用");
  ...
  console.log("   4. 使用 CREATE2 部署 NextswapV3Pool");
  console.log("   5. 新池从 deployer 读取参数并初始化");
  console.log("   6. deploy 方法清除临时参数");

  // 验证池合约
  const pool = await ethers.getContractAt("NextswapV3Pool", poolAddress);
