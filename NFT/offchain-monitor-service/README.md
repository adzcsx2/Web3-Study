...
- **批量处理机制** - 智能批量处理，支持高并发场景（50 个事件/批次）
- **区块确认监控** - 6 个区块确认机制，确保交易最终性
- **链重组检测** - 自动检测和处理链重组事件
- **自动重连机制** - 最多 10 次重连，指数退避策略
- **并发控制** - 最大 10 个并发处理任务
...
- **确认要求** - 6 个区块确认，确保交易最终性
- **实时跟踪** - 每 30 秒检查一次待确认事件
- **批量更新** - 高效的批量确认区块数更新
...
- **连接状态** - 每 5 分钟检查 Supabase 和 WebSocket 连接
- **服务状态** - 监控事件监听器是否正常运行
- **异常恢复** - 检测到异常时自动尝试恢复
- **告警机制** - 异常情况下的详细日志记录
...
服务启动后会在配置的端口（默认 3000）上提供 HTTP 健康检查端点：

- **健康检查**: `GET /health`

  - 返回服务健康状态、Supabase 连接状态、监听器状态
  - Railway 使用此端点验证服务是否正常运行
  - 响应示例:
    ```json
    {
      "status": "healthy",
