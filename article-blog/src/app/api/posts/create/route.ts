...
import { PostType } from '@/types/book'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (profileError || !userProfile || !['admin', 'author'].includes(userProfile.role)) {
      return NextResponse.json(
...
// 处理文章标签
async function handlePostTags(supabase: ReturnType<typeof createServerClient>, postId: string, tags: string[]) {
  // 获取或创建标签
  const tagIds: string[] = []
