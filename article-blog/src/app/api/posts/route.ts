...
import { PostListReq } from '@/types/book'
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 解析查询参数
    const sortParam = searchParams.get('sort') as 'created_at' | 'updated_at' | 'view_count' | 'like_count' | null
    ...
    if (params.author) {
      query = query.or(`author_username.ilike.%${params.author}%,author_nickname.ilike.%${params.author}%`)
    }
    ...
    const processedPosts = posts?.map(post => ({
      ...post,
      tags: (post as { post_tags?: Array<{ tags: { name: string; slug: string } }> }).post_tags?.map(pt => pt.tags) || []
    })) || []
