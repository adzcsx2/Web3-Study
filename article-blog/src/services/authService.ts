...
export class AuthService {
  ...
  // 验证用户权限
  static async hasPermission(userId: string, requiredRole: USER_ROLE): Promise<boolean> {
    const { data: profile } = await supabase
      .from('user_profiles')
      .select('role')
      .eq('id', userId)
    ...
    return roleHierarchy[profile.role as USER_ROLE] >= roleHierarchy[requiredRole]
  }
